...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage: scripts/maintenance/run_dailymed_1k_test_gate.sh [--deepinfra-key <key>] [--start-production]
...
run_ingest_test_namespace() {
  ...
  run_logged "${run_name}" env \
    ...
    python3 scripts/dailymed_ingest_lib.py \
...
run_logged "download_test_xml" python3 scripts/baseline/download_dailymed.py --output-dir "${DAILYMED_XML_DIR}"
...
if [[ "${START_PRODUCTION}" == "true" ]]; then
  ...
  run_logged "download_production_xml" python3 scripts/baseline/download_dailymed.py --output-dir "${DAILYMED_XML_DIR}"
